                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.coupons.update", [$coupon->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('select_product') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="select_product_id" id="select_product_id" required>
                                @foreach($select_products as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('select_product_id') ? old('select_product_id') : $coupon->select_product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('discount') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="discount" id="discount" value="{{ old('discount', $coupon->discount) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('valid_from') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="valid_from" id="valid_from" value="{{ old('valid_from', $coupon->valid_from) }}">
                        ...
                        <div class="form-group {{ $errors->has('valid_to') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="valid_to" id="valid_to" value="{{ old('valid_to', $coupon->valid_to) }}">
