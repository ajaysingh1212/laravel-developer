                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.brands.update", [$brand->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $brand->title) }}" required>
                            @if($errors->has('title'))
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status" required>
                                ...
                                @foreach(App\Models\Brand::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $brand->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
