        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.addshiping.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.addshipings.update", [$addshiping->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('type') ? 'has-error' : '' }}">
                            <label class="required" for="type">{{ trans('cruds.addshiping.fields.type') }}</label>
                            <input class="form-control" type="text" name="type" id="type" value="{{ old('type', $addshiping->type) }}" required>
                            @if($errors->has('type'))
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $addshiping->price) }}" step="1" required>
                            @if($errors->has('price'))
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status">
                                ...
                                @foreach(App\Models\Addshiping::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $addshiping->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
